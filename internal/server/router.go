...
func NewRouter(db *database.DB) http.Handler {
	...
	mux.Handle("POST /api/v1/projects", authMw(http.HandlerFunc(projectHandler.GetProject)))
	mux.Handle("POST /api/v1/projects/create", authMw(http.HandlerFunc(projectHandler.CreateProject)))
	...
	mux.Handle("POST /api/v1/console/crawl-status/{job_id}", authMw(http.HandlerFunc(botBuilder.GetCrawlJobStatus)))
	...
	mux.Handle("POST /api/v1/console/re-embed/{project_id}", authMw(http.HandlerFunc(botBuilder.ReEmbedChunks)))
	mux.Handle("POST /api/v1/console/embed-plan/{project_id}", authMw(http.HandlerFunc(botBuilder.GetEmbedPlan)))
	mux.Handle("POST /api/v1/console/embed-pause/{job_id}", authMw(http.HandlerFunc(botBuilder.PauseEmbedJob)))
	mux.Handle("POST /api/v1/console/embed-status/{job_id}", authMw(http.HandlerFunc(botBuilder.GetEmbedJobStatus)))
	mux.Handle("POST /api/v1/console/documents/{project_id}", authMw(http.HandlerFunc(botBuilder.GetDocuments)))
	mux.Handle("POST /api/v1/console/chunks/{project_id}", authMw(http.HandlerFunc(botBuilder.GetChunks)))
